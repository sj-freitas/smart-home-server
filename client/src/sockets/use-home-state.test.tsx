...
jest.mock('socket.io-client', () => ({
  io: jest.fn(() => {
    ...
    mockSocketOn = jest.fn((event: string, handler: (...args: unknown[]) => void) => {
      if (!mockSocketEventHandlers[event]) mockSocketEventHandlers[event] = [];
...
describe('useHomeState', () => {
  ...
  it('updates state when the socket emits state:update', () => {
    ...
    const payload: HomeState = { name: 'My Home', subTitle: '', logo: '', rooms: [] };
