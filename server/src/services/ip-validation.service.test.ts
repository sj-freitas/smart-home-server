...
const makeContext = (clientIp: string | null): Pick<RequestContext, 'clientIp'> =>
  ({ clientIp } as any);
...
describe('IpValidationService', () => {
  ...
  it('always returns false when AUTH_ALWAYS_DISALLOW_THE_IP is "true"', () => {
    ...
    const svc = new IpValidationService(makeContext('192.168.1.5') as RequestContext, makeConfig());
    expect(svc.isRequestAllowedBasedOnIP()).toBe(false);
  });

  describe('when no IP is configured in home config', () => {
    it('allows every request', () => {
      const svc = new IpValidationService(makeContext('8.8.8.8') as RequestContext, makeConfig());
